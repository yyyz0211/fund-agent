...
export function IndustrySectorTable({ snap }: { snap: MarketSnapshot }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <h3 className="font-semibold mb-2 text-sm text-gray-700">行业涨跌幅</h3>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-gray-400 text-xs"><th className="text-left pb-1">板块</th><th className="text-right pb-1">涨跌幅</th></tr>
          </thead>
          <tbody>
            {sectors.map(s => (
      ...
      <div>
        <h3 className="font-semibold mb-2 text-sm text-gray-700">行业资金流向</h3>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-gray-400 text-xs"><th className="text-left pb-1">板块</th><th className="text-right pb-1">净流入(亿)</th></tr>
          </thead>
          <tbody>
            {flows.map(f => (
